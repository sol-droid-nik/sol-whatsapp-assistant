...
function buildKbQuery(message, st = {}) {
  ...
  // ВРЕМЕННО: не приклеиваем предыдущий вопрос, чтобы не путать темы
  return m;
}
...
async function answerFromKb(query, userLang = "fi") {
  ...
  const prompt = `
Ты ассистент SOL. 
1) Используй тексты из документов SOL как главный источник.
2) Если вопрос про зарплату, больничный, отпуск, договор,
   и в документах нет точного ответа — честно скажи, что
   информации нет, и посоветуй обратиться к руководителю или HR.
3) Если вопрос про ПРАКТИЧЕСКУЮ УБОРКУ (как мыть, чем оттереть,
   какие движения, какая последовательность),
   а в документах нет точной инструкции, ты МОЖЕШЬ дать
   общий профессиональный совет по уборке:
   - не выдумывай названия средств SOL и точные дозировки,
     если их нет в текстах;
   - можно говорить об общих категориях ("кислотное средство
     для унитазов", "универсальное щёлочное средство" и т.п.);
   - обязательно напоминай про использование СИЗ и чтение
